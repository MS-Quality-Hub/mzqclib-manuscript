...
class Run:
    ...
    n_ms2id: int = 0  # from PSM crema confidence estimate
    n_pep: int = 0
    n_prot: int = 0
...
def getMetricSourceFramesBase(run: mzml.MzML) -> pd.DataFrame:
    ...
    ms2_only_padlist = ['precursor_int', 'precursor_c', 'precursor_mz', 'activation_method', 'activation_energy',
                        'isolation_window_target_mz', 'isolation_window_lower_offset', 'isolation_window_upper_offset']
    for spectrum in run:
        nid = spectrum['id']
        ...
        if len(spectrum['scanList']['scan']) > 1:
            logging.warn(
                "Scan {} has more than one spectrum, ignoring all but first.".format(nid))
        if next(iter(spectrum['scanList']['scan']))['scanWindowList']['count'] < 1:
            logging.warn("Scan {} has no window, ignoring.".format(nid))
            continue
        if next(iter(spectrum['scanList']['scan']))['scanWindowList']['count'] > 1:
            logging.warn(
                "Scan {} has more than one window, ignoring all but first.".format(nid))

        preset_min_mz = next(iter(next(iter(spectrum['scanList']['scan']))[
                             'scanWindowList']['scanWindow']))['scan window lower limit']
        preset_max_mz = next(iter(next(iter(spectrum['scanList']['scan']))[
                             'scanWindowList']['scanWindow']))['scan window upper limit']
        ...
        inj_msec = next(iter((spectrum['scanList']['scan'])))[
            'ion injection time']
        ...
        if "MSn spectrum" in spectrum:
            ...
            if len(next(iter(spectrum["precursorList"]['precursor']))["selectedIonList"]['selectedIon']) > 1:
                logging.warn(
                    "Scan {} has more than one selected ion, ignoring all but first.".format(nid))

            data_acquisition['precursor_int'].append(next(iter(next(iter(spectrum["precursorList"]['precursor']))[
                                                     "selectedIonList"]['selectedIon'])).get('peak intensity', np.nan))
            data_acquisition['precursor_c'].append(next(iter(next(iter(spectrum["precursorList"]['precursor']))[
                                                   "selectedIonList"]['selectedIon']))['charge state'])
            data_acquisition['precursor_mz'].append(next(iter(next(iter(spectrum["precursorList"]['precursor']))[
                                                    "selectedIonList"]['selectedIon']))['selected ion m/z'])
            data_acquisition['activation_method'].append(next(iter([x[0] for x in next(iter(
                spectrum["precursorList"]['precursor']))['activation'].items() if x[1] == ''])))
            data_acquisition['activation_energy'].append(next(
                iter(spectrum["precursorList"]['precursor']))['activation']['collision energy'])
            data_acquisition['isolation_window_target_mz'].append(next(iter(
                (spectrum["precursorList"]['precursor'])))['isolationWindow']['isolation window target m/z'])
            data_acquisition['isolation_window_lower_offset'].append(next(iter(
                (spectrum["precursorList"]['precursor'])))['isolationWindow']['isolation window lower offset'])
            data_acquisition['isolation_window_upper_offset'].append(next(iter(
                (spectrum["precursorList"]['precursor'])))['isolationWindow']['isolation window upper offset'])
...
def load_mzml(mzml_path: str) -> Run:
    ...
    r = doc.xpath('/x:indexedmzML/x:mzML/x:referenceableParamGroupList/x:referenceableParamGroup/x:cvParam',
                  namespaces={'x': "http://psi.hupo.org/ms/mzml"})
    ms = pronto.Ontology(psi_ms_url, import_depth=0)
    cv_instruments = {x.id for x in ms['MS:1000031'].subclasses().to_set()}
    mzml_instrument = {tag.attrib.get("accession", None) for tag in r if tag.attrib.get(
        "accession", None) in cv_instruments}
    if len(mzml_instrument) > 1:
        logging.warn(
            "Provided mzML has more than one instrument registered, ignoring all but first.")
    itype = ms.get(next(iter(mzml_instrument)))
    # Start time
    s = doc.xpath('/x:indexedmzML/x:mzML/x:run[@startTimeStamp]',
                  namespaces={'x': "http://psi.hupo.org/ms/mzml"})
    strt = pd.to_datetime(s[0].attrib.get(
...
def load_ids(run: Run, crux_tide_index: str, crux_tide_search: str, tide_index: str, tide_search: str, fdr: int = 1) -> Run:
    ...
    """
    PROTON_AMU = 1.0073
    tide_target_file = os.path.join(
        crux_tide_search, tide_search+'.target.txt')
    ...
    with open(tide_target_file, 'r') as targets, open(tide_decoy_file, 'r') as decoys:
        ...
        filtered_tide_target = pd.read_csv(targets, sep='\t').sort_values(
            by='xcorr score', ascending=False).groupby('scan').head(1)
        filtered_tide_decoy = pd.read_csv(decoys, sep='\t').sort_values(
            by='xcorr score', ascending=False).groupby('scan').head(1)
        # crema fdr
        ...
        results = psms.assign_confidence(
            score_column="xcorr score", desc=True, pep_fdr_type="peptide-only", threshold=fdr/100)
        prt_df = results.confidence_estimates['proteins']
        # add delta ppm per peptide and fix column names
        pep_df = results.confidence_estimates['peptides'].rename(columns={"scan": "scan_id"}).merge(filtered_tide_target.rename(columns={"scan": "scan_id"})[
            ['scan_id', 'sequence', 'charge', 'peptide mass', 'spectrum precursor m/z']], how="inner", on=['scan_id', 'sequence']).rename(columns={"spectrum precursor m/z": "experimentalMassToCharge"})
        pep_df['calculatedMassToCharge'] = (
            pep_df['peptide mass']+PROTON_AMU*pep_df['charge'])/pep_df['charge']
        psm_df = results.confidence_estimates['psms'].rename(columns={"scan": "scan_id"})
    ...
    run.n_ms2id = psm_df[psm_df['accept'] == True]['scan_id'].nunique()

    logging.debug("Registered proteins "+str(run.n_prot))
    logging.debug("Registered peptides "+str(run.n_pep))
    logging.debug("Registered id MS2 "+str(run.n_ms2id))
    return run
...
def construct_mzqc(run: Run, quality_metric_values: List[qc.QualityMetric]) -> qc.MzQcFile:
    ...
    qc.MzQcFile
            An mzqc file object
    """
    infi1 = qc.InputFile(name=run.mzml_path, location=run.mzml_path,
                         fileFormat=qc.CvParameter("MS:1000584", "mzML format"))
    infi1.fileProperties.append(qc.CvParameter(
        "MS:1003151", "SHA-256", run.checksum))
    infi1.fileProperties.append(qc.CvParameter(
        run.instrument_type.id, run.instrument_type.name))
    infi1.fileProperties.append(qc.CvParameter(
...
def calc_metric_missedcleavage(run: Run) -> qc.QualityMetric:
    ...
    """
    ids_only = run.base_df.merge(run.id_df, how="inner", on='scan_id')
    ids_only["MS:1003044"] = [len(fastaparser.cleave(
        seq, 'trypsin'))-1 for seq in ids_only['sequence'].to_list()]

    mc_or_greater = 3  # i.e. 3 missed cleavages or greater
    ids_only["mc_group"] = np.where(ids_only["MS:1003044"] >= mc_or_greater, str(
        mc_or_greater), ids_only["MS:1003044"])
    mc_agg = ids_only.groupby("mc_group").agg({'native_id': 'count'}).reset_index(
    ).rename(columns={"native_id": "NCIT:C150827", "mc_group": "MS:1003044"})
...
def calc_metric_deltam(run: Run) -> Tuple[qc.QualityMetric]:
    ...
    logging.debug(
        "theor. "+str(ids_only['calculatedMassToCharge'].to_list()[:10]))
    logging.debug(
        "measu. "+str(ids_only['experimentalMassToCharge'].to_list()[:10]))

    metric_value_mean = qc.QualityMetric(
        accession="MS:4000178", name="precursor ppm deviation mean", value=ids_only['mass_error_ppm'].mean())
    metric_value_std = qc.QualityMetric(
        accession="MS:4000179", name="precursor ppm deviation sigma", value=ids_only['mass_error_ppm'].std())
...
def calc_metric_idrtquarters(run: Run) -> qc.QualityMetric:
    ...
    idf_rtsort['quarter'] = np.repeat(np.arange(1, 5), [
                                      quarter_ids]*3 + [idf_rtsort.shape[0] - quarter_ids*3])  # label row with quarter

    quarter_interval_durations = idf_rtsort.groupby(
        'quarter')['RT'].max() - idf_rtsort.groupby('quarter')['RT'].min()
...
# help="The file with the spectra to analyse"
@click.argument('mzml_input', type=click.Path(exists=True, readable=True))
# help="The file with the spectrum identifications to analyse"
@click.argument('crux_tide_index', type=click.Path(exists=True, readable=True))
# help="The file with the spectrum identifications to analyse"
@click.argument('crux_tide_search', type=click.Path(exists=True, readable=True))
# help="The output path for the resulting mzqc"
@click.argument('mzqc_output', type=click.Path(writable=True, dir_okay=False))
...
def simple_qc_metric_calculator(mzml_input, crux_tide_index, crux_tide_search, mzqc_output, fdr, tide_index, tide_search, dev, log):
    ...
    try:
        ...
        run = load_ids(run, crux_tide_index, crux_tide_search,
                       tide_index, tide_search, fdr)
    ...
    quality_metric_values = [*calc_metric_deltam(run), calc_metric_ioncollection(run),
                             calc_metric_missedcleavage(
                                 run), *calc_metric_idrate(run),
    ...
    if dev:
        for n, df in [("base data frame", run.base_df), ("identifications data frame", run.id_df)]:
            quality_metric_values.append(
                qc.QualityMetric(accession="MS:4000005", name=n, value=df.where(
                    (pd.notnull(df)), None).to_dict(orient='list'))
            )
